...
import { ARTIFACT_TABS, DEFAULT_ARTIFACT_TABS, ENTRY_POINT_CONFIG, ENTRY_POINT_DETAIL_TABS, type TabProps } from './artifact-config';
import { ArtifactApiDefinition, ServiceResources } from './ArtifactTabs';
import { ArtifactTypeSelector } from './ArtifactDetail';

function EntryPointDetail({ selected, onOpenDrawerTab }: { selected: { artifact: GqlArtifact; artifactType: string; envId: string; componentId: string; projectId: string }; onOpenDrawerTab: (tab: string) => void }) {
  ...
  const handleToggleStatistics = (checked: boolean) => {
    ...
    updateStatisticsStatus.mutate(
      ...
      { onError: () => setStatisticsEnabled(prev) },
    );
  };
...
export function Environment({
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setRefreshError(err instanceof Error ? err.message : 'Failed to refresh artifacts');
    } finally {
