...
class H2RepositoryImplementation {
    ...
    *BaseRepository;
    ...
    isolated function getDisplayNameById(string? userId) returns string? {
        ...
        types:User|error user = self.getUserDetailsById(userId);
        ...
        check from types:Environment env in envStream
            do {
                environments.push({
                    ...
                    createdBy: self.getDisplayNameById(env.createdBy),
                    updatedAt: env.updatedAt,
                    updatedBy: self.getDisplayNameById(env.updatedBy)
                });
            };
        return environments;
    }
    ...
    public isolated function getEnvironmentsByIds(string[] environmentIds) returns types:Environment[]|error {
        ...
        stream<types:Environment, sql:Error?> envStream = self.dbClient->query(query);
    ...
    public isolated function getEnvironmentIdsByTypes(boolean hasProdAccess, boolean hasNonProdAccess) returns string[]|error {
        ...
        stream<record {|string environment_id;|}, sql:Error?> envStream = self.dbClient->query(query);
    ...
    public isolated function getEnvironmentById(string environmentId) returns types:Environment|error {
        ...
        self.dbClient->query(`SELECT environment_id, name, description, is_production, created_at, updated_at FROM environments WHERE environment_id = ${environmentId}`);
    ...
    VALUES (${envId}, ${environment.name}, ${environment.description}, ${environment.isProduction}, ${environment.createdBy})`;
        var result = self.dbClient->execute(insertQuery);
        ...
        return check self.getEnvironmentById(envId);
    }
    ...
    public isolated function getEnvironmentIdByName(string environmentName) returns string|error {
        stream<record {|string environment_id;|}, sql:Error?> envStream = self.dbClient->query(`
    ...
    public isolated function getComponentIdByName(string componentName) returns string|error {
        stream<record {|string component_id;|}, sql:Error?> componentStream = self.dbClient->query(`
    ...
    public isolated function getProjectIdByName(string projectName) returns string|error {
        stream<record {|string project_id;|}, sql:Error?> projectStream = self.dbClient->query(`
    ...
    public isolated function deleteEnvironment(string environmentId) returns error? {
        ...
        var result = self.dbClient->execute(deleteQuery);
        if result is sql:Error {
    ...
    public isolated function getRuntimes(string? status, string? runtimeType, string? environmentId, string? projectId, string? componentId) returns types:Runtime[]|error {
        ...
        stream<types:RuntimeDBRecord, sql:Error?> runtimeStream = self.dbClient->query(query);

        check from types:RuntimeDBRecord runtime in runtimeStream
            do {
                runtimeList.push(check self.mapToRuntime(runtime));
            };
    ...
    public isolated function getRuntimesByAccessibleEnvironments(types:UserContext userContext) returns types:Runtime[]|error {
        ...
        if userContext.isSuperAdmin {
            types:Runtime[] runtimeList = [];
            stream<types:RuntimeDBRecord, sql:Error?> runtimeStream = self.dbClient->query(`
        ...
        types:Environment[]|error allEnvironments = self.getEnvironments();
    ...
    `);
        ...
        return check self.mapToRuntime(runtimeRecords[0]);
    }
    ...
    public isolated function getServicesForRuntime(string runtimeId) returns types:Service[]|error {
        ...
        stream<types:Service, sql:Error?> serviceStream = self.dbClient->query(`
    ...
    `);
        ...
        check from types:Service serviceRecord in serviceStream
            do {
                ...
                serviceList.push(check self.mapToService(serviceRecord, runtimeId));
            };
    ...
    public isolated function getListenersForRuntime(string runtimeId) returns types:Listener[]|error {
        ...
        stream<types:Listener, sql:Error?> listenerStream = self.dbClient->query(`
    ...
    public isolated function getApisForRuntime(string runtimeId) returns types:RestApi[]|error {
        ...
        stream<types:RestApi, sql:Error?> apiStream = self.dbClient->query(`
    ...
    public isolated function getProxyServicesForRuntime(string runtimeId) returns types:ProxyService[]|error {
        ...
        stream<types:ProxyServiceRecordInDB, sql:Error?> proxyStream = self.dbClient->query(`
    ...
    public isolated function getEndpointsForRuntime(string runtimeId) returns types:Endpoint[]|error {
        ...
        stream<types:EndpointRecordInDB, sql:Error?> endpointStream = self.dbClient->query(`
    ...
    public isolated function getInboundEndpointsForRuntime(string runtimeId) returns types:InboundEndpoint[]|error {
        ...
        stream<types:InboundEndpoint, sql:Error?> inboundStream = self.dbClient->query(`
    ...
    public isolated function getSequencesForRuntime(string runtimeId) returns types:Sequence[]|error {
        ...
        stream<types:SequenceRecordInDB, sql:Error?> sequenceStream = self.dbClient->query(`
    ...
    public isolated function getTasksForRuntime(string runtimeId) returns types:Task[]|error {
        types:Task[] taskList = [];
        stream<types:TaskRecordInDB, sql:Error?> taskStream = self.dbClient->query(`
    ...
    public isolated function getTemplatesForRuntime(string runtimeId) returns types:Template[]|error {
        ...
        stream<types:Template, sql:Error?> templateStream = self.dbClient->query(`
    ...
    public isolated function getMessageStoresForRuntime(string runtimeId) returns types:MessageStore[]|error {
        ...
        stream<types:MessageStoreRecordInDB, sql:Error?> storeStream = self.dbClient->query(`
    ...
    public isolated function getMessageProcessorsForRuntime(string runtimeId) returns types:MessageProcessor[]|error {
        ...
        stream<types:MessageProcessorRecordInDB, sql:Error?> processorStream = self.dbClient->query(`
    ...
    public isolated function getLocalEntriesForRuntime(string runtimeId) returns types:LocalEntry[]|error {
        ...
        stream<types:LocalEntryRecordInDB, sql:Error?> entryStream = self.dbClient->query(`
    ...
    public isolated function getDataServicesForRuntime(string runtimeId) returns types:DataService[]|error {
        ...
        stream<types:DataService, sql:Error?> serviceStream = self.dbClient->query(`
    ...
    public isolated function getCarbonAppsForRuntime(string runtimeId) returns types:CarbonApp[]|error {
        ...
        stream<types:CarbonAppRecordInDB, sql:Error?> appStream = self.dbClient->query(`
    ...
    public isolated function getDataSourcesForRuntime(string runtimeId) returns types:DataSource[]|error {
        ...
        stream<types:DataSource, sql:Error?> sourceStream = self.dbClient->query(`
    ...
    public isolated function getConnectorsForRuntime(string runtimeId) returns types:Connector[]|error {
        ...
        stream<types:Connector, sql:Error?> connectorStream = self.dbClient->query(`
    ...
    public isolated function getRegistryResourcesForRuntime(string runtimeId) returns types:RegistryResource[]|error {
        ...
        stream<types:RegistryResourceRecordInDB, sql:Error?> resourceStream = self.dbClient->query(`
    ...
    public isolated function getSystemInfoForRuntime(string runtimeId) returns types:SystemInfo[]|error {
        ...
        stream<types:SystemInfo, sql:Error?> infoStream = self.dbClient->query(`
    ...
    public isolated function mapToRuntime(types:RuntimeDBRecord runtimeRecord) returns types:Runtime|error {
        ...
        types:Service[] serviceList = check self.getServicesForRuntime(runtimeRecord.runtime_id);

        // Get listeners for this runtime
        types:Listener[] listenerList = check self.getListenersForRuntime(runtimeRecord.runtime_id);
        ...
        if runtimeRecord.runtime_type == "MI" {
            apiList = check self.getApisForRuntime(runtimeRecord.runtime_id);
            proxyList = check self.getProxyServicesForRuntime(runtimeRecord.runtime_id);
            endpointList = check self.getEndpointsForRuntime(runtimeRecord.runtime_id);
            inboundList = check self.getInboundEndpointsForRuntime(runtimeRecord.runtime_id);
            sequenceList = check self.getSequencesForRuntime(runtimeRecord.runtime_id);
            taskList = check self.getTasksForRuntime(runtimeRecord.runtime_id);
            templateList = check self.getTemplatesForRuntime(runtimeRecord.runtime_id);
            storeList = check self.getMessageStoresForRuntime(runtimeRecord.runtime_id);
            processorList = check self.getMessageProcessorsForRuntime(runtimeRecord.runtime_id);
            entryList = check self.getLocalEntriesForRuntime(runtimeRecord.runtime_id);
            dataServiceList = check self.getDataServicesForRuntime(runtimeRecord.runtime_id);
            appList = check self.getCarbonAppsForRuntime(runtimeRecord.runtime_id);
            sourceList = check self.getDataSourcesForRuntime(runtimeRecord.runtime_id);
            connectorList = check self.getConnectorsForRuntime(runtimeRecord.runtime_id);
            resourceList = check self.getRegistryResourcesForRuntime(runtimeRecord.runtime_id);
            infoList = check self.getSystemInfoForRuntime(runtimeRecord.runtime_id);
        }
        ...
        return {
            ...
            environment: check self.getEnvironmentById(runtimeRecord.environment_id),
            component: check self.getComponentById(runtimeRecord.component_id),
    ...
    public isolated function mapToService(types:Service serviceRecord, string runtimeId) returns types:Service|error {
        ...
        stream<types:ResourceRecord, sql:Error?> resourceStream = self.dbClient->query(`
    ...
    `;
        sql:ExecutionResult _ = check self.dbClient->execute(updateQuery);
    }
    ...
    public isolated function processDeltaHeartbeat(types:DeltaHeartbeat deltaHeartbeat) returns types:HeartbeatResponse|error {
        ...
        sql:ExecutionResult|error timestampResult = self.dbClient->execute(`
        UPDATE runtimes
        SET last_heartbeat = ${currentTimeStr}, status = 'RUNNING'
        WHERE runtime_id = ${deltaHeartbeat.runtime}
    `);
        ...
        transaction {
            ...
            stream<types:ControlCommand, sql:Error?> commandStream = self.dbClient->query(`
    ...
    isolated function validateHeartbeatData(types:Heartbeat heartbeat) returns error? {
        ...
        string|error envId = self.getEnvironmentIdByName(heartbeat.environment);
        if envId is error {
            return error(string `Invalid environment configuration detected: ${heartbeat.environment}`, envId);
        }
        heartbeat.environment = envId;

        string|error projectId = self.getProjectIdByName(heartbeat.project);
        ...
        string|error componentId = self.getComponentIdByName(heartbeat.component);
    ...
    isolated function validateComponentRuntimeConsistency(string componentId, types:Artifacts newArtifacts) returns error? {
        ...
        stream<record {|string runtime_id;|}, sql:Error?> runtimeStream = self.dbClient->query(`
    ...
    `);
        ...
        types:Service[] referenceServices = check self.getServicesForRuntime(referenceRuntimeId);
        types:Listener[] referenceListeners = check self.getListenersForRuntime(referenceRuntimeId);

        // Compare new artifacts with reference
        error? servicesValidation = self.validateServicesConsistency(referenceServices, newArtifacts.services);
        ...
        error? listenersValidation = self.validateListenersConsistency(referenceListeners, newArtifacts.listeners);
    ...
    isolated function validateServicesConsistency(types:Service[] referenceServices, types:Service[] newServices) returns error? {
        ...
        foreach types:Service newService in newServices {
            ...
            // Validate resources
            error? resourceValidation = self.validateResourcesConsistency(refService.resources, newService.resources);
    ...
    isolated function deleteExistingArtifacts(string runtimeId) returns error? {
        ...
        _ = check self.dbClient->execute(`DELETE FROM runtime_services WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_listeners WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM service_resources WHERE runtime_id = ${runtimeId}`);

        // Delete MI artifacts for all runtimes (will be empty for non-MI runtimes)
        _ = check self.dbClient->execute(`DELETE FROM runtime_apis WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_proxy_services WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_endpoints WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_inbound_endpoints WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_sequences WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_tasks WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_templates WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_message_stores WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_message_processors WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_local_entries WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_data_services WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_carbon_apps WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_data_sources WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_connectors WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_registry_resources WHERE runtime_id = ${runtimeId}`);
        _ = check self.dbClient->execute(`DELETE FROM runtime_system_info WHERE runtime_id = ${runtimeId}`);
    }
    ...
    isolated function insertRuntimeArtifacts(types:Heartbeat heartbeat) returns error? {
        ...
        check self.deleteExistingArtifacts(heartbeat.runtime);
        ...
        // Insert MI artifacts if present
        check self.insertMIArtifacts(heartbeat);

        // Insert additional MI artifacts not covered by insertMIArtifacts
        check self.insertAdditionalMIArtifacts(heartbeat);
    }
    ...
    public isolated function processHeartbeat(types:Heartbeat heartbeat) returns types:HeartbeatResponse|error {
        ...
        isNewRegistration = check self.upsertRuntime(heartbeat, currentTimeStr);
        ...
        transaction {
            ...
            check self.insertRuntimeArtifacts(heartbeat);
            ...
            if isNewRegistration {
                error? validationResult = self.validateComponentRuntimeConsistency(heartbeat.component, heartbeat.artifacts);
    ...
    public isolated function createProject(types:ProjectInput project, types:UserContext userContext) returns types:Project|error? {
        ...
        transaction {
            ...
            sql:ExecutionResult _ = check self.dbClient->execute(insertQuery);
            ...
            if !userContext.isSuperAdmin {
                ...
                string prodRoleId = check self.getOrCreateRole(projectId, types:PROD, types:ADMIN);
                sql:ExecutionResult _ = check self.dbClient->execute(
                ...
                string nonProdRoleId = check self.getOrCreateRole(projectId, types:NON_PROD, types:ADMIN);
                sql:ExecutionResult _ = check self.dbClient->execute(
                `INSERT INTO user_roles (user_id, role_id, assigned_by)
        ...
        return self.getProjectById(projectId);
    }
    ...
    public isolated function hasProjectComponents(string projectId) returns boolean|error {
        ...
        stream<record {int component_count;}, sql:Error?> resultStream = self.dbClient->query(query);
        ...
        check from types:ComponentInDB component in componentStream
            do {
                ...
                components.push({
                    ...
                    createdBy: self.getDisplayNameById(component.component_created_by),
                    updatedBy: self.getDisplayNameById(component.component_updated_by)
                });
            };
        return components;
    }
    ...
    public isolated function getComponentById(string componentId) returns types:Component|error {
        stream<types:ComponentInDB, sql:Error?> componentStream =
        self.dbClient->query(`SELECT c.component_id, c.project_id, c.name as component_name, c.description as component_description, 
                                c.created_by as component_created_by, c.created_at as component_created_at, c.updated_at as component_updated_at,
                                c.updated_by as component_updated_by,
                                p.org_id as project_org_id, p.name as project_name, p.version as project_version, 
                                p.created_date as project_created_date, p.handler as project_handler, p.region as project_region,
    ...
    public isolated function getUserDetailsById(string userId) returns types:User|error {
        ...
        types:User|sql:Error user = self.dbClient->queryRow(
    ...
    public isolated function getUserRoles(string userId) returns types:Role[]|error {
        ...
        types:Role[] roles = [];
        stream<types:Role, sql:Error?> roleStream = self.dbClient->query(
    ...
    isolated function getOrCreateRole(string projectId, types:EnvironmentType environmentType, types:PrivilegeLevel privilegeLevel) returns string|error {
        ...
        stream<record {|string role_id;|}, sql:Error?> roleStream = self.dbClient->query(
        ...
        types:Project project = check self.getProjectById(projectId);
    ...
    public isolated function updateUserRoles(string userId, types:RoleAssignment[] roleAssignments) returns error? {
        ...
        transaction {
            ...
            foreach types:RoleAssignment assignment in roleAssignments {
                ...
                string roleId = check self.getOrCreateRole(
    ...
    public isolated function getAllUsers() returns types:UserWithRoles[]|error {
        ...
        stream<types:User, sql:Error?> userStream = self.dbClient->query(
        `SELECT user_id, username, display_name, is_super_admin, is_project_author, created_at, updated_at
         FROM users
        ...
        check from types:User user in userStream
            do {
                ...
                types:Role[]|error rolesResult = self.getUserRoles(user.userId);
    ...
    public isolated function getUsersByProjectIds(string[] projectIds) returns types:UserWithRoles[]|error {
        ...
        stream<types:User, sql:Error?> userStream = self.dbClient->query(query);
    ...
    public isolated function updateUserProfile(string userId, string displayName) returns error? {
        ...
        types:User|error existingUser = self.getUserDetailsById(userId);
    ...
    public isolated function validateRefreshToken(string tokenHash) returns types:User|error {
        ...
        |}|sql:Error refreshToken = self.dbClient->queryRow(
        ...
        sql:ExecutionResult|sql:Error updateResult = self.dbClient->execute(
        ...
        types:User|error user = self.getUserDetailsById(refreshToken.user_id);
        if user is error {
    ...
    public isolated function checkProjectCreationEligibility(int orgId, string orgHandler) returns types:ProjectCreationEligibility|error {
        ...
        stream<record {}, sql:Error?> projectCountStream = self.dbClient->query(query);
    ...
    public isolated function checkProjectHandlerAvailability(int orgId, string projectHandlerCandidate) returns types:ProjectHandlerAvailability|error {
        ...
        stream<record {}, sql:Error?> handlerCountStream = self.dbClient->query(query);
        ...
        if !isHandlerUnique {
            ...
            while counter <= 10 { // Limit to 10 attempts to avoid infinite loop
                ...
                stream<record {}, sql:Error?> candidateStream = self.dbClient->query(alternateQuery);
