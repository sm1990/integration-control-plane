...
export default function ComponentGroupDetail(): JSX.Element {
  ...
  const { data: groups, isLoading: loadingGroups } = useGroups(orgHandler, projectId || undefined, componentId);

  if (loadingProject || loadingComponent || loadingGroups)
    return (
      <PageContent>
        <Loading />
      </PageContent>
  ...
  return (
    <PageContent>
      <GroupDetailView orgHandler={orgHandler} group={group} projectId={projectId} componentId={componentId} showUsers={false} onBack={() => navigate(componentAccessControlUrl(orgHandler, projectHandler, componentHandler, 'groups'))} />
    </PageContent>
  );
}
